    ...
    $carieksternal = mysql_query("SELECT status FROM selesai WHERE waktu='$waktu' AND no='$no' AND status='Vendor/Supplier'");
      ...
      if ($insert){
        if($divisi == 'FINANCE'){
          ...
          echo "<script language='javascript'>";
          echo "alert('Untuk pembayaran External, Harap lampirkan dokumen terkait seperti Invoice, Berita Acara DSB. Untuk di ajukan ke Ibu Ina Puspito untuk mendapatkan persetujuan')";
          echo "</script>";
